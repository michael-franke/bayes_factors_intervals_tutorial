...
pitch in Hz stored in the variable \texttt{pitch}.
...
parameter \texttt{context1} should also be weakly informative
...
the other parameters, but you always should critically reflect on all of
your priors.
...
The posterior samples (red distribution) suggests that the majority of
plausible values after seeing the data are positive, or in other words,
informal contexts elicit larger pitch values. Negative values are not
very plausible posterior values, but also not completely implausible.
Compared to our prior probability (green distribution) for which roughly
50\% of posteriors are negative, this decrease in plausibility of
negative values is quite noteworthy already.
