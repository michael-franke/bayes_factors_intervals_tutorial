...
a given probability threshold. By convention, the JND is often defined
as the point where listener accuracy exceeds the arbitrary threshold of
75\%. When we say ``just noticeable difference'' in the following we
mean that latter probabilistic notion, as we would like to use the idea
of regions at which the perceptual system is unlikely to register a
stimulus difference as a means of justifying a region of practical
equivalence. For example, while classic work by Klatt
...
begin with a JND value that is somewhat conservative in relation to
Klatt (\citeproc{ref-klatt1973discrimination}{1973}) adn somewhat
liberal in relation to Turner et al.
(\citeproc{ref-turner2019perception}{2019}): We assume that pitch values
between \texttt{-10} and \texttt{10} are meaningless. Bear with us, we
...
With a Bayes factor of around 3.07, the data provides some evidence in
favor of the alternative hypothesis that the effect of \texttt{context}
on \texttt{pitch} is outside of the ROPE.
...
pronounced, so that we would only speak of moderate evidence in favor of
the alternative hypothesis and not strong or decisive evidence.
...
Hz. We also assume the following five prior values for the width of the
standard deviation of the critical parameter (centered on zero): 10, 15,
20, 25, 30. These are all sensible prior widths assuming that medium to
strong pitch effects in either direction are plausible.
...
{[}klatt1973discrimination;Turner et al.
(\citeproc{ref-turner2019perception}{2019}){]}.
...
quantitative systems under investigation {[}e.g.; Scheel
(\citeproc{ref-scheel2022most}{2022}); Coretta et al.
(\citeproc{ref-coretta2023multidimensional}{2023}); Woensdregt et al.
(\citeproc{ref-woensdregt2024lessons}{2024}){]}. The presented form of
hypothesis testing is easy to understand, but does require to think
deeply about prior quantitative assumptions as well as what it means for
observations to be meaningfully different. That is neither trivial nor
easy. But we would like to encourage everybody to engage in exactly this
thinking to better understand our data and how they might link to our
